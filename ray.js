class Ray {
	...
	cast() {
		...
		for (let cast of this.casts) {
			...
			let t = Infinity;
			for (let barrier of this.barriers) {
				let intersection = Ray.getIntersection(this.position.x, this.position.y, cast.x, cast.y, barrier.pointA.x, barrier.pointA.y, barrier.pointB.x, barrier.pointB.y);
				if (intersection) {
					if (intersection.t < t) {
						t = intersection.t;
						nearestIntersection = intersection;
						castedBarrier = barrier;
					}
				}
			}
